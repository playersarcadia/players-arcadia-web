...
describe('Header', () => {
  ...
  it('toggles mobile menu when menu button is clicked', async () => {
    ...
    const mobileNavs = screen.getAllByText('Home').map(link => link.closest('nav')).filter(Boolean)
    const mobileNav = mobileNavs.find(nav => nav?.classList.contains('md:hidden'))
    expect(mobileNav).toHaveClass('max-h-0', 'opacity-0')
    
    await user.click(menuButton)
    
    // After click, mobile menu should be visible
    const mobileNavsAfter = screen.getAllByText('Home').map(link => link.closest('nav')).filter(Boolean)
    const mobileNavAfter = mobileNavsAfter.find(nav => nav?.classList.contains('md:hidden'))
